...
async function run() {
  ...
  for (const phase of (overview.tournament_phases || [])) {
    if (phase.round_type === 'SWISS' && phase.number_of_rounds) {
      totalSwissRounds += phase.number_of_rounds;
    }
    for (const r of (phase.rounds || [])) {
      allRounds.push(r);
    }
  }
  if (totalSwissRounds === 0) totalSwissRounds = 13;

  const activeRounds = allRounds.filter(r => r.status === 'COMPLETE' || r.status === 'IN_PROGRESS');
  console.log('Active rounds for Singapore Regional:', activeRounds.map(r => ({ round: r.round_number, status: r.status, id: r.id })));
  ...
  const payload = {
    tournamentId: '889532',
    tournamentName: '🏆 Riftbound Regional Qualifier - Singapore',
  ...
  fs.writeFileSync('singapore_data.json', JSON.stringify(payload, null, 2), 'utf8');
  console.log('Saved singapore_data.json successfully with', metaData.length, 'legends and', playersList.length, 'players!');
}
